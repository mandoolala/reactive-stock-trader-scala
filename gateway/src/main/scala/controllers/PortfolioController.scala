...
import stocktrader.broker.api.{BrokerService, OrderSummary}
  ...
  def getSummary(portfolioId: PortfolioId, includeOrderInfo: Boolean, includePrices: Boolean) = Action.async { _ =>
    val summaryView: Future[PortfolioSummary] = for {
      ...
      completedOrders <- {
        ...
        else Future.successful(Seq.empty) // FIXME: Used nulls in Java version
      }
    } yield {
      val equityHoldings = {
        if (includePrices) model.holdings.map { holding =>
          EquityHolding(
            ...
            sharePrice = 0 // FIXME: Should this be optional?
          )
        }
        else Seq.empty // FIXME: Used nulls in Java version
      }
      PortfolioSummary(
