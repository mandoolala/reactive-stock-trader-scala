...
import {gatewayBaseURL} from '@/common/config';
import {activePortfolio} from '@/common/portfolio';

const transfer = axios.create({
  baseURL: gatewayBaseURL + '/api/transfer',
...
export function getAllTransfersFor(portfolioId) {
  const url = new URL('/api/transfer/' + portfolioId, gatewayBaseURL);
...
export function submitTransfer(transferRequest) {
  ...
  return axios({
    ...
    url: gatewayBaseURL + '/api/transfer',
    data: formData
  });
}
